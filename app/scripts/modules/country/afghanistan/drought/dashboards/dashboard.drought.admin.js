					...
					$scope.model = {
						...
						header: {
							...
							title: {
								...
								'title': 'Drought Response' + ' | ' + $scope.dashboard.user.admin0pcode + ' | ' + $scope.dashboard.user.cluster + ' | ' + $scope.dashboard.user.organization +' '+$route.current.params.year,
							},
							subtitle: {
								...
								downloads: [						
								 {
									...
									hover: 'Download Drought Beneficiaries as CSV',
										request: angular.merge({}, ngmDroughtHelper.getRequest('/drought/afghanistan/beneficiaries/data', 'data', false), { data: { report: 'drought_beneficiaries_' + $route.current.params.status_plan } }),
							...
							columns: [{
								...
								widgets: [{
									...
									config: {
										...
										reset: '#/response/afghanistan/drought/dashboard/' + $route.current.params.status_plan,
										getCurrentMonth: '#/response/afghanistan/drought/dashboard/' + $route.current.params.status_plan + '/' + moment().year() + '/all/all/all/all/' + (moment().month() + 1) + '/' + moment().startOf('month').format('YYYY-MM-DD') + '/' + moment().format('YYYY-MM-DD'),
