		...
		return {
			...
			getMonthRows: function () {				
				...
				for (i = 0; i < 12; i++) {				
					...
					rows.push({
						...
						'active': i+1,
						'class': 'grey-text text-darken-2 waves-effect waves-teal waves-teal-lighten-4',
						'href': dashboard.url + '/' + dashboard.statusPlan+ '/' + dashboard.year + '/' + dashboard.cluster + '/' + dashboard.province + '/' + dashboard.district + '/' + dashboard.organization + '/' + (i+1) + '/' + start_date + '/' + end_date
					});
				}
				return {
			...
			getSubtitle:function () {
				...
				if( dashboard.month !== 'all'){
					var monthName = moment.months(dashboard.month-1);
				}else{
