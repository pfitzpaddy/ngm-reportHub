/**
 * @ngdoc function
 * @name ngmReportHubApp.controller:DashboardAccessibilityCtrl
 * @description
 * # LoginCtrl
 * Controller of the ngmReportHub
 */
angular.module('ngmReportHub')
	.controller('DashboardAccessibilityCtrl', ['$scope', '$http', '$route', '$location', '$q', '$filter', '$timeout', 'ngmUser', 'ngmData', function ($scope, $http, $route, $location, $q, $filter, $timeout, ngmUser, ngmData) {
		this.awesomeThings = [
			'HTML5 Boilerplate',
			'AngularJS',
			'Karma'
		...
		$scope.dashboard = {
			...
			accessibilityHref: '/immap/drr/accessibility/' + $route.current.params.province,
			...
			getProvinceRows: function() {
				...
				angular.forEach($scope.dashboard.data, function(d, key){
					rows.push({
						...
						'href': '#/immap/drr/accessibility/' + key
					});
				});
			...
			getDistrictRows: function() {
				...
				angular.forEach($scope.dashboard.districts, function(d, key){
					rows.push({
						...
						'active': key,
						'class': 'grey-text text-darken-2 waves-effect waves-teal waves-teal-lighten-4',
						'href': '#/immap/drr/accessibility/' + $route.current.params.province + '/' + key
					});
				});
			...
			setDashboard: function(data) {
				...
				// title
				var title = 'iMMAP | Accessibility | ' + $scope.dashboard.data[$route.current.params.province].prov_name;
				var subtitle = 'Accessibility Key Indicators for ' + $scope.dashboard.data[$route.current.params.province].prov_name + ' Province';
				...
				if ( !$route.current.params.district ) {
					...
					$scope.dashboard.accessibilityHref += '/' + $route.current.params.district;
					...
					$scope.dashboard.center = {
						...
						zoom: $scope.dashboard.districts[ $route.current.params.district ].zoom ? $scope.dashboard.districts[ $route.current.params.district ].zoom : 10,
					}
				...
				$scope.model = {
					name: 'drr_accessibility_dashboard',
					header: {
						div: {
							...
							widgets: [{
								...
								config: {
									...
									tabs: [{
										...
										title: 'Accessibility',
										'class': 'active',
										href: $scope.dashboard.accessibilityHref
									},{
										col: 's12 m4',
				...
				if ( $scope.dashboard.flag === 'currentProvince' ) {
					$scope.model.menu[1] = {
						'search': true,
						'id': 'search-accessibility-district',
						'icon': 'place',
						'title': 'District',
						'class': 'blue white-text',
